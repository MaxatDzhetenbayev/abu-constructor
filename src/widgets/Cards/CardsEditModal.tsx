...
// Убери неиспользуемый код и структуры. Я хочу создать хук useWidgetmanager, внутри которого находилась бы вся главная логика для работы с бэкенд частью, а здесь оставались бы только структуры самой карточки, которые могу передаваться в хук, и возвращались бы данные, которые могут использоваться здесь. Это нужно, чтобы следующие хуки создавались быстрее и без повторения логики. Тут самое главное переиспользуемость и расширяемость. Также можешь разделить код на компоненты для удобства. 